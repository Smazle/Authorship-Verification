...
Since we answer all problems $problems$ and $answers$ are the same in our case and we therefore get that the F1 measure is the same as an accuracy, 
\begin{equation}
...
\cite{pacheco2015}, we start by computing our feature set, where the
features used match the ones used in out experiments. More specifically,\\
...
These features was also created to make a \gls{UBM} as described in the section
\nameref{subsec:Generalising Random Forest}.
These two data sets, are then encoded using according to equation 
...
fault, with \texttt{n\_estimators} being the exception, as it is set to 1000, 
to combat the variance.

At this point we encode the feature set created from the test data, against the
\gls{UBM}, which is then fed to our trained random forest to get a set our
set of prediction.
This resulted in an accuracy of 0.604, on the pan 2015 dataset.

On the other hand, we also chose to train our model using the alternate 
subtraction encoding, which doesn't make use of the \gls{UBM}. This however
under-performed relative to the \gls{UBM} approach, with an accuracy of 
0.584, as can be seen in figure \ref{fig:forest_roc}.
