...
% TODO: Consider explaining TP and FN.
\begin{align}
    TPR &= \frac{TP}{TP + FN} \\
    TNR &= \frac{TN}{TN + FP}
\end{align}
...
Applying the algorithm on the PAN 2015 dataset, gave a accuracy of 0.5185,
...
configurations are shown in Figure \ref{fig:extended_delta_result}.
...
\begin{figure}
    ...
    The result consist of 2 numbers in the format $a, b$. $a$ corresponds to the
    accuracy obtained with the configuration and $b$ is the number of opponents
    that obtained that accuracy. Results that beat our baseline results are
    shown in bold.}
    \label{fig:extended_delta_result}
\end{figure}
\end{landscape}
...
We use the RBF kernel and we choose hyperparameters by cross validation. Each
configuration of features will use different hyperparameters. The cross
validation is performed by looping through the list of authors. For each author
we performing a grid search for value of $C \in \{1 \cdot 10^{-2}, 1 \cdot
10^{0}, \dots, 1 \cdot 10^{10}\}$ and $\gamma \in \{1 \cdot 10^{-9}, 1 \cdot
10^{-7}, \dots, 1 \cdot 10^{3}\}$. The best values for each author are found via
leave one out cross validation. The final $C$ and $\gamma$ values are chosen as
the configurations used most often by the authors. After we have found the best
hyperparameters we run the classifier over all authors 100 times with those
hyperparameters. The mean accuracy over the 100 runs is then computed.
