...
% TODO: Describe the PAN 2013 performance measures as well.
...
the unknown texts belonged to the same author as the known texts. The PAN 2015
task were ranked according to two measures. The \gls{AUROC} and the c@1 measure
...
$n_u$ is the number of unanswered problems. The final rating of the solutions is
the product of the two measures. The \gls{AUROC} is a measure of discrimination.
That is, it measures the ability of a solution to distinguish between texts
written by the same author and texts written by another author. An \gls{AUROC}
score is generally considered excellent when between 0.9 and 1, good when
between 0.8 and 0.9, fair when between 0.7 and 0.8, poor when between 0.6
and 0.7 and a failure when between 0.5 and 0.6. The c@1 measure is chosen
since it measures performance in the binary case. The c@1 measure does not use
probabilities but classifies everything above 0.5 as a yes everything below as
a no and a 0.5 as a don't know. In this section when we speak about performance
we mean the product of the two measures for a specific method. Since we only
use the English texts in the PAN 2015 dataset we will also only report the
